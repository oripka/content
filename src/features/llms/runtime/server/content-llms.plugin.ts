...
import { createDocumentGenerator, isVisibleAutoGeneratedContent, prepareContentSections } from './utils'
...
export default defineNitroPlugin((nitroApp: NitroApp) => {
  ...
  nitroApp.hooks.hook('llms:generate', async (event, options) => {
    ...
    for (const index in options.sections) {
      ...
      let docs = await query.all() as PageCollectionItemBase[]
      if (section.__nuxt_content_auto_generate) {
        docs = docs.filter(isVisibleAutoGeneratedContent)
      }
  ...
  function getDocumentLink(link: string, collection: string, options: ModuleOptions) {
    const contentRawMarkdown = (options as unknown as { contentRawMarkdown: false | { rewriteLLMSTxt: boolean, excludeCollections: string[], privateCollections: string[], includeCollections: string[] }, encryptionEnabled?: boolean })?.contentRawMarkdown
    const encryptionEnabled = (options as unknown as { encryptionEnabled?: boolean })?.encryptionEnabled
    if (
      contentRawMarkdown === false
      || contentRawMarkdown?.rewriteLLMSTxt === false
      || !isRawMarkdownEnabledForCollection(collection, contentRawMarkdown, {
        encryptionEnabled,
        collectionMeta: (collections as Record<string, { private?: boolean }>)[collection],
      })
    ) {
