...
import { isRawMarkdownEnabledForCollection } from '../../../../utils'

export default eventHandler(async (event) => {
  const config = useRuntimeConfig(event)
  const llmsConfig = config.llms as { contentRawMarkdown: false | { excludeCollections: string[], privateCollections: string[], includeCollections: string[] }, encryptionEnabled?: boolean }
  ...
  const _collections = Object.entries(collections as Record<string, ResolvedCollection>)
    .filter(([key, value]) => value.type === 'page'
      && isRawMarkdownEnabledForCollection(key, llmsConfig?.contentRawMarkdown, {
        encryptionEnabled: llmsConfig?.encryptionEnabled,
